...
var (
	...
	// rootdir is the path to the project root directory, calculated by two directories up relative to this file
	rootdir = filepath.Join(filepath.Dir(sourcefile), ".."+string(os.PathSeparator)+"..") + string(os.PathSeparator)
...
func main() {
	...
	if !cmdExists("psql") && !cmdExists("docker") {
		printAndExit("Either psql or docker needs to be installed but both not found, exiting")
	}
	...
	if len(sorted.data) > 0 && (hasCustomFiles || *cleanFlag) {
		fmt.Printf("\nLoading data files\n")
...
func sortDirs() sortedFiles {
	var sorted sortedFiles
	var err error
	sorted.views, err = filepath.Glob(filepath.Join("sql", "views", "*.sql"))
	if err != nil {
		panic(err)
	}
	sorted.triggers, err = filepath.Glob(filepath.Join("sql", "triggers", "*.sql"))
	if err != nil {
		panic(err)
	}
	sorted.functions, err = filepath.Glob(filepath.Join("sql", "functions", "*.sql"))
	if err != nil {
		panic(err)
	}
	sorted.data, err = filepath.Glob(filepath.Join("sql", "data", "*sql"))
	if err != nil {
		panic(err)
	}
	sorted.tests, err = filepath.Glob(filepath.Join("sql", "tests", "*sql"))
	if err != nil {
		panic(err)
	}
	return sorted
...
func sortFiles(args []string) sortedFiles {
	...
	for _, arg := range args {
		...
		switch {
		case strings.Contains(filename, filepath.Join("sql", "views")):
			sorted.views = append(sorted.views, arg)
		case strings.Contains(filename, filepath.Join("sql", "triggers")):
			sorted.triggers = append(sorted.triggers, arg)
		case strings.Contains(filename, filepath.Join("sql", "functions")):
			sorted.functions = append(sorted.functions, arg)
		case strings.Contains(filename, filepath.Join("sql", "data")):
			sorted.data = append(sorted.data, arg)
		case strings.Contains(filename, filepath.Join("sql", "tests")):
			sorted.tests = append(sorted.tests, arg)
		default:
			fmt.Printf("could not identify %s as a view, trigger, function or data script, defaulting to view\n", arg)
			sorted.views = append(sorted.views, arg)
		}
	}
	return sorted
...
// Load a bunch of sql files into the database, then run the postprocessor
// functions (if any) on each file. A postprocessor function takes in filename
// and does something with it. The postprocessing functions are how we run the
// SQL function 'plpgsql_check_function()' on each of the function files.
...
func runTests(filenames []string) error {
	...
	start := func() {
		fmt.Println()
		fmt.Println("-- Start the plan (needed before any pgTAP tests can be run)")
	...
	for _, filename := range filenames {
		...
		for _, function := range testfunctions {
			...
			if !testpassed {
				if *testallFlag {
					// If a test failed but user wants to run all tests, continue
					sometestfailed = true
					continue
				}
				return fmt.Errorf("Error when running test file: %s", filename)
...
func testFunction(db *sql.DB, function string) (testpassed bool, err error) {
	...
	for _, str := range strs {
		...
		if strings.HasPrefix(str, "not ok") {
			// But lines containing '# SKIP' or '# TODO' should be ignored.
			// Tests can be marked as todo using the todo_start() and
			// todo_end() functions.
